...
- Keybindings are ratified as in the design guide: `Enter` submit/queue, `Ctrl+C` interrupt/clear, `Ctrl+P` Palette, `?` help, `Ctrl+E`/`Ctrl+Y` scroll, `Ctrl+T` toggle Turn Diff, vim normal/insert editing with `j`/`k` history. The Palette is an interactive list of Models grouped by Provider with type-to-filter and the current Selection marked.
- The TUI activates only when stdout is a TTY and `TERM` is not `dumb`; the inline renderer remains the non-TTY path with its v0.1 behavior (indicator transitions, Command Report, Tool Output, refusal reporting). Colors come from semantic tokens only; no state is conveyed by color alone; `NO_COLOR` disables color.
