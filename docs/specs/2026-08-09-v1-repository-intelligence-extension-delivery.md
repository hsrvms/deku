...
- The **tui** module renders the activity seam with bubbletea, lipgloss, and bubbles, per `docs/guides/tui-design.md`: a Transcript pane (incremental `TextDelta` rendering, scrollable) that hosts a Turn Diff block inside the Agent's response section (auto-opens on the first `Change` event of a Turn; shows the cumulative per-file working-tree diff computed by the renderer from the Change path set via `git diff`, with new files rendered as full-content addition entries and per-file (200 lines) and total (1000 lines) caps with a truncation note; persists in the Transcript as the Turn's history), a status bar (Working Indicator with label, glyph, and color; active Tool; current Provider/Model), and a single-line vim-mode input. Approval renders in the input area as the Command Report prompt with the available decisions; the status bar shows awaiting Approval.
- The Transcript pane is a list of structured messages (kind + text) rendered from the activity seam, not raw streamed text: streamed model text stays plain and incremental; user requests render right-aligned in a semantic token color; section separators appear above each user request and above the Agent's response (the exchange boundary) and frame each Tool Output and Command Report block, adjacent separators collapsing into one line and a separator never sharing a line with message text. The pane drops the inline renderer's plain-text echo of a typed block (the Write immediately following the typed event, which the Agent emits first), so each block appears exactly once; the inline renderer is unchanged and still formats the same facts its own way.
...
- The status bar's active Tool is part of the activity seam: the Sink gains `ActiveTool(name)`, emitted by the Agent at the moment a Tool begins executing, so no renderer derives which Tool is running from anything else. The shell also records the `Change` set the seam delivers; the Turn Diff block renders it.
- Keybindings are ratified as in the design guide: `Enter` submit/queue, `Ctrl+C` interrupt/clear, `Ctrl+P` Palette, `?` help, `Ctrl+E`/`Ctrl+Y` scroll, `Ctrl+T` toggle Turn Diff, vim normal/insert editing with `j`/`k` history. The Palette is an interactive list of Models grouped by Provider with type-to-filter and the current Selection marked; its choice applies the per-Session Selection override through the same `/model` command path, asynchronously, so the program loop never blocks on the Agent's Selection lock while a Turn runs. Interrupt semantics are ratified: `Ctrl+C` cancels the running Turn's context — the Agent exits with a cancellation error the shell reports as `turn interrupted` — and clears the input line when idle; `Ctrl+D` (the inline path's EOF) quits the TUI. The `?` help binding is a normal-mode key: the modeless-safety rule forbids binding any character that can be typed in insert mode, so `?` types in insert mode and opens the help overlay in normal mode. A message submitted with `Enter` while a Turn runs is queued and rendered as a user message immediately; the queue drains in order as each running Turn completes, and an interrupted Turn yields to its queued messages. Approval renders in the input area as the Command Report prompt (`Approve? [y/n]`) while the status bar shows awaiting Approval, and decisions are delivered to the waiting gate through a pipe so neither side touches the raw terminal.
