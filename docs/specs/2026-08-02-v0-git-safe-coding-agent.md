...
20. As a developer, I want the final response to distinguish Validation results from Git recoverability, so that a successful commit is not mistaken for proof that the repository works. The distinction is surfaced by the CLI presentation layer, which renders the Agent's `TurnResult` fields—Validation outcome, Agent Commit id, and stash reference—as separate lines distinct from the model's assistant text, so a commit is reported as a recoverable boundary and never as proof of correctness.
...
The v0 benchmark is implemented as an opt-in integration test at the Agent seam (`TestV0Benchmark`). It runs only when `DEKU_BENCHMARK=1` and an OpenAI-compatible Provider is configured through `DEKU_PROVIDER_ENDPOINT`, `DEKU_PROVIDER_API_KEY`, and `DEKU_PROVIDER_MODEL`; otherwise it is skipped so ordinary `go test ./...` and `make ci` runs never make network calls or claim model quality. The seed is a committed `example.com/textstats` module of roughly 30 Go source files across six packages plus a command, with exactly one failing test (`stats.TestMean`) whose root cause is a single buggy line in `stats.Mean`; the fix is a one-line exact-match Edit. The benchmark drives one Turn with all tools auto-approved, enforces Agent Commits in `on` mode, and asserts that `go test ./...` passes, that the Agent Commit contains only the fixed file (`stats/mean.go`), and that the measured Provider-call count is at most 8 with at most 60,000 total billed tokens. Provider-call count is measured by wrapping the `Chat` interface; billed tokens come from the Provider-reported usage aggregated across Steps in `TurnResult`. A deterministic integration test (`TestFixtureFullTurnFixesAndCommits`) exercises the same fixture through the Agent seam with a scripted Provider and real temporary Git repository, so the end-to-end flow is verified without a real model.
The benchmark was verified against a live compatible Provider: it completed one Turn in 5 Provider calls with 10,682 billed input and 757 billed output tokens (11,439 total), well within the v0 targets of 8 calls and 60,000 tokens, and produced an Agent Commit containing only `stats/mean.go`.
