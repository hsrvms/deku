...
- The **Agent module** is the primary deep module. Its interface accepts one user request in a Session and drives the entire Turn: prompt assembly, Provider interaction, Tool execution, Approval, continuation Steps, and final reporting. Callers do not coordinate model loops or Tool results. The first conversation slice implements one Step with a base system prompt and no Tools; later slices add the Repository Map, Tools, Approval, and continuation behavior.
...
- The **Session module** persists an immutable, append-only JSONL message log under the Deku home directory and reconstructs Session history when resumed. The initial CLI creates timestamped unique Session IDs, stores one role/content message per JSONL record under `~/.deku/sessions/<id>.jsonl`, and resumes an ID with `deku --resume <id>`.
